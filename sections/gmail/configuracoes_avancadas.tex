...
\begin{enumerate}
    ...
    \item Ações de passar o \gls{cursor}: Habilita ou desabilita ícones de ação rápida que aparecem ao passar o mouse sobre um e-mail.
    ...
    \item Recursos inteligentes: Permite que informações do Gmail sejam usadas em outros \gls{aplicativos} Google, como \Gls{chat} e \Gls{meet}.
    \item Recursos inteligentes do Google \Gls{workspace}: Permite que informações de e-mails sejam utilizadas em outros serviços do Google \Gls{workspace}.
...
\begin{enumerate}
    \item \Gls{chat}: Ativa ou desativa o Google \Gls{chat} no Gmail.
    \item Configurações do \Gls{chat}: Ajusta a posição do \Gls{chat} na tela e permite 
        ativar ou desativar o histórico de conversas.
    \item Configurações do \Gls{meet}: Permite gerenciar preferências de reuniões, 
        como disponibilidade e integração de convites, diretamente pelo Gmail.
    \item \Gls{meet}: Mostra ou oculta a seção do Google \Gls{meet} na barra lateral.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Atalhos de teclado personalizados: Habilita a criação de combinações 
        ...
        preferem atalhos.
    \item Ícone de mensagem não lida: Exibe no ícone da \gls{aba} do navegador, a 
...
\begin{itemize}
    ...
    \item As mensagens enviadas \gls{offline} ficam na caixa de saída até que o Gmail 
...
A \gls{aba} Temas permite ao usuário personalizar a aparência visual do Gmail, 
