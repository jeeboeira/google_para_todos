...
\begin{itemize} 
  ...
  \item \textbf{Comentários}: habilita um modo especial de visualização que não permite edições no projeto, mas apenas a inclusão de comentários nos \textbf{slides} que podem ser visualizados pelos usuários com permissões de edição (atalho [Ctrl + Alt + M]). 
...
Ao clicar sobre essa opção, a apresentação entrará em modo ``tela cheia'' para exibição do conteúdo dos \textbf{slides}. Clique em [Esc] para sair desse modo de visualização.
...
\textbf{(Imagem ver.png)}

\subsection{Menu de zoom}

Essa função fornece algumas opções que permitem ao usuário ampliar (atalho [Ctrl + ``='']) ou reduzir ([Ctrl + ``-'']) a visualização do \textbf{slide} no documento.
