...
\begin{itemize}
  ...
  \item \textbf{Mostrar todos os comentários}: abre uma guia lateral que lista todos os comentários feitos no \gls{arquivo} da sua apresentação. 
\end{itemize}
...
Mostra duas barras com medidas em cm (centímetros) na extremidade superior e lateral esquerda do \gls{slide}. Útil para redimensionamentos de elementos que exigem precisões milimétricas.
...
\begin{dica}
  Ao arrastar as guias, note o número exibido na tela: ele corresponde à distância, em cm, que a guia está posicionada em relação à extremidade superior do documento (horizontal) ou à extremidade esquerda (vertical). Use essa informação para ajustá-las adequadamente.
\end{dica}
...
A guia ``Ponteiros ao vivo'' possui duas opções que você pode habilitar ou desabilitar: ``Mostrar ponteiros dos colaboradores'' e ``Mostrar meu ponteiro''. Isso refere-se à visualização do \gls{cursor} do \textbf{mouse} no modo colaborativo de edição, em tempo real.

\begin{dica}
  No modo ``Apresentação de slides'', em tela cheia, aperte \tecla{L} para transformar o \gls{cursor} do \textbf{mouse} em uma ferramenta visual semelhante a um laser, útil para destacar figuras, gráficos ou textos importantes.
\end{dica}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=.39\textwidth]{images/slides/ver/Imagem 1.png}
    \caption{Tira de filme}
\end{figure}

\subsection{Menu de zoom}

Essa função fornece algumas opções que permitem ao usuário ampliar (atalho \tecla{Ctrl + ``=''}) ou reduzir (\tecla{Ctrl + ``-''}) a visualização do \textbf{\gls{slide}} no documento.
