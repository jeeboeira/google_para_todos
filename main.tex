...
\addbibresource{references/slides.bib}
\addbibresource{references/gmail/introducao_gmail.bib}
\addbibresource{references/gmail/dicas_e_boas_praticas.bib}
\addbibresource{references/documentos/trabalhando_com_estilos.bib}
\addbibresource{references/agenda/introducao_google_agenda.bib}
\addbibresource{references/agenda/dicas_e_boas_praticas.bib}
...
% Ensure subsection titles are left-aligned (prevent full justification)
...
% Background for cover pages
\newcommand{\faixa}{%
	\setchaptercolor
\begin{tikzpicture}[remember picture,overlay]
    % pontos no topo: 0%, 25%, 50%, 75%, 100%
    \coordinate (P0) at (current page.north west);
    \coordinate (P1) at ($(current page.north west)!0.25!(current page.north east)$);
    \coordinate (P2) at ($(current page.north west)!0.50!(current page.north east)$);
    \coordinate (P3) at ($(current page.north west)!0.75!(current page.north east)$);
    \coordinate (P4) at (current page.north east);

    % faixa de 0 a 25%
    \fill[catDocs]
        (P0) rectangle ([yshift=-0.8cm]P1);

    % faixa de 25 a 50%
	\fill[catPlanilhas]
        (P1) rectangle ([yshift=-0.8cm]P2);

    % faixa de 50 a 75%
    \fill[catSlides]
        (P2) rectangle ([yshift=-0.8cm]P3);

    % faixa de 75 a 100% (se quiser usar)
    \fill[catGmail]
        (P3) rectangle ([yshift=-0.8cm]P4);
\end{tikzpicture}
	\vspace*{-1.5cm}
}
% ------------------------------------------------------------
...
\begin{document}
	...
	\input{frontmatter/contracapa.tex}
...
\chapter{Introdução}
\input{sections/introducao/apresentacao_apostila.tex}
\input{sections/introducao/workspace_escolha_ideal.tex}
\input{sections/introducao/sobre_as_ferramentas.tex}
\input{sections/introducao/conclusao.tex}
% 
...
\input{sections/extras/ferramentas_utilizadas.tex}
